...
if (navigator.appVersion.indexOf("Win")!=-1) {
  ...
  <title>◊(capitalize-first-letter here-title) | Obesidad en Línea</title>
        ...
        ◊div[#:class "nav-outer" #:id "bottom"]{
            ◊div[#:class "nav-inner"]{
                  ◊span[#:id "left"]{◊(if (eq? here 'toc.html)
                    ...
                    ◊span{◊xref["/index.html"]{OEL inicio}}
                    ◊span[#:id "right"]{◊(if next-page ◊xref{◊(select 'title next-page)} ◊xref["https://corporacioncovsa.blogspot.com/p/nosotros.html"]{corporación covsa})}}})})

◊(->html (body (default-body)))
                      

</html>
<!-- El contenido sobre nutrición de esta página web le pertence a David Villafuerte pero la plantilla con la que se creó esta página web es decir el código .html, .css y .js se basa en una versión anterior del libro Typography for Lawyers de Matthew Butterick que se programó con Pollen (pollenpub.com) -->